...
通过 lua 的 `print(package.path)` 可以查看 lua 的模块搜索路径，其中是不包含 `~/.config/nvim/lua` 的，但 neovim 硬编码了这个模块搜索路径，优先级比 `package.path` 更高，所以这个目录下的模块也能被正常加载。
...
\$VIMINIT 环境变量会被读取为一条 Ex 命令行，Vim 和 Neovim 都会检查这个变量。

启动顺序（两者相同）：

1. 检查 $VIMINIT 环境变量，如果存在，执行其中的 Ex 命令（并跳过后续配置加载）
2. 如果 \$VIMINIT 不存在，检查 `$XDG_CONFIG_HOME/nvim/init.vim` (或 `init.lua`)
